...
<div class="container requirements-container">
    ...
    {{-- If totalKW exceeds 50 kW, show contact support message --}}
    ...
    @else
        <div class="system-types">
            {{-- Hybrid System --}}
            @if($requiredSystemSize['hybridInverterSize'] !== 'Not available')
                ...
                @elseif($requiredSystemSize['onGridInverterSize'] !== 'Not available')
                <div class="system-card">
                    <h2>Hybrid System</h2>
                    <p>This system is not suitable. Hybrid inverter is available only for loads up to 12 kW.</p>
                </div>
            @endif

            {{-- On-Grid System --}}
            @if($requiredSystemSize['onGridInverterSize'] !== 'Not available')
                <div class="system-card">
                    <h2>On-Grid System</h2>
                    <p><strong>Inverter Size:</strong> {{ $requiredSystemSize['onGridInverterSize'] }}</p>
                    <p><strong>Number of Panels:</strong> {{ $requiredSystemSize['onGridPanels'] ?? 'Not calculated' }}</p>
                    <p><strong>Estimated Annual Generation:</strong> {{ $requiredSystemSize['onGridAnnualGeneration'] ?? 'Not calculated' }}</p>
                </div>
            @endif
        </div>
    @endif
